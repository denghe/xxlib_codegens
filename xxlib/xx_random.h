...
namespace xx {
	...
	// 标识内存可移动
	template<>
	struct IsTrivial<Random, void> : std::true_type {};

	template<>
	struct SFuncs<Random, void> {
		static inline void Append(std::string& s, Random const& in) noexcept {
			xx::Append(s, "{ \"type\":\"xx.Random\", \"inext\":", in.inext, "}");
		}
	};
