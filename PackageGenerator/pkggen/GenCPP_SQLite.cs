...
namespace " + iface.Namespace + @" {");
    ...
    public:
		...
		inline " + iface.Name + @"(xx::SQLite::Connection& conn)
            : conn(conn)" + sb2 + @"
        {}");
            ...
            ,");
                    ...
                    sb2.Append("typename " + p.Name._ToFirstUpper() + "Type_" + p.ParameterType.Name);
                }

                sb.Append(@"


        xx::SQLite::Query query_" + fn + @";" + f._GetDesc()._GetComment_Cpp(8));
                if (sb2.Length > 0)
                {
                    sb.Append(@"
        template<" + sb2 + @">");
